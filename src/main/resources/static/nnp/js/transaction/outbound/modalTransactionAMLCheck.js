...
$(document).ready(async function(e) {
    ...
    // 이벤트 위임 방식으로 변경
    $(document).on('click', '#approveBtn', async function(e) {
        e.preventDefault();
        ...
        const beneficiaryTable = new DataTable("#AMLCheckbeneficiaryTable", {
            rowId: "No.",
            columns: [
                { data: "id", title: "No." },
                { data: "name", title: "Name" },
        ...
        const customerType = $('#customerType').text();
        const senderName = $('#senderName').text();
        const beneficiaryName = $('#beneficiaryName').text();
        const customerID = $('#senderCustomerId').text();

        const promises = [
            check(senderName, 'AMLCheckSenderTable', 'senderCheckName', senderTable, 'Sender'),
            check(beneficiaryName, 'AMLCheckbeneficiaryTable', 'beneficiaryCheckName', beneficiaryTable, 'Beneficiary')
        ];

        if(customerType === 'COMPANY') {
            $("#ceocheck").removeClass("d-none");

            const ceoPromise = new Promise((resolve, reject) => {
                $.ajax({
                    type: 'GET',
                    url: `/customer/${customerID}/ceo`,
                    beforeSend: function (xhr) {
                        xhr.setRequestHeader(header, token);
                    },
                    success: (data) => {
                        if (data.authorizedPersonName) {
                            resolve(
                                check(data.authorizedPersonName, 'AMLCheckSenderCeoTable', 'senderCeoCheckName', senderCeoTable, 'CEO')
                            );
                        } else {
                            $("#AMLCheckSenderCeoTable").addClass("d-none");
                            document.getElementById("senderCeoCheckName").innerHTML =
                                `Can't found <strong>CEO</strong>`;
                            resolve(null); // null을 반환하여 Promise 처리
                        }
                    },
                    error: (xhr, error, thrown) => {
                        console.error("Error:", error);
                        alert("An error occurred while getting authorized Person Name.");
                        reject(error);
                    }
                });
            });
            promises.push(ceoPromise); // CEO 관련 promise 추가
        }
        ...
        $('#AMLCheckbeneficiaryTable tbody').on('click', 'tr', function () {
            const row = beneficiaryTable.row(this);
            const isExpanded = $(this).hasClass('shown');

            $('#AMLCheckbeneficiaryTable tbody tr').removeClass('shown');
            $('.detail-row').remove();
            showDetailData(row, isExpanded, $(this));
        });
