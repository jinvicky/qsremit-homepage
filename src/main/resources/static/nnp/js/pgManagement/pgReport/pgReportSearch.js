$(document).ready(function () {
    ...
    const fetchData = (month) => {
        $.ajax({
            url: `/api/pgManagement/pgReport/search/${month}`,
            type: 'GET',
            dataType: 'json',
            success: function (data) {
                ...
                if (Array.isArray(data) && data.length > 0) {
                    data.forEach((item) => {
                        const formattedAmount = Number(item.totalSettlementAmount || 0).toLocaleString('ja-JP');
                        const formattedDate = item.mailSentAt
                            ? moment(item.mailSentAt).format('YYYY-MM-DD HH:mm:ss')
                            : '-';
                        tbody.append(`
                        <tr data-type-id="${item.accountDepositTypeId}"
                            data-type-name="${item.accountDepositTypeCustomerType}">
                            <td class="text-center">${item.accountDepositTypeReceiptNo}</td>
                            <td class="text-center">${unescapeSpecialCharacters(item.accountDepositTypeCustomerType)}</td>
     ...
     * 모달창
     */
    let companyName, companyEmail, companyNo;
    $('#pgReportTable').on('click', '.validationBtn', function() {
        ...
        $.ajax({
            ...
            success: function(response) {
                companyName = `${response.customerName}`;
